                        ...
                        >
                            ...
                            <template v-slot:body>
                                <view style="display: flex">
                                    <text
                                        style="font-size: 0.9em; color: #f0ad4e"
                                        >{{
                            ...
                            <template v-slot:footer>
                                <view style="display: flex; flex-wrap: wrap">
                                    <uni-tag
                                        ...
                                        "
                                        :text="
                                            getConsultText(tutor.lecture_label)
                                        "
                                        type="primary"
                                    >
                                    </uni-tag>
    ...
    methods: {
        ...
        filter() {
            ...
            if (this.consultvalue !== "") {
                data = data.filter(
                    (lecture) => lecture.lecture_label === this.consultvalue
                );
            }
            if (this.payvalue !== "") {
                const payRanges = [
                ...
                data = data.filter(
                    (lecture) =>
                        lecture.lecture_price >= min &&
                        lecture.lecture_price < max
                );
            }
            if (this.timevalue !== "") {
                const timeRanges = [
                ...
                data = data.filter(
                    (lecture) =>
                        lecture.lecture_duration >= min &&
                        lecture.lecture_duration < max
                );
            }
